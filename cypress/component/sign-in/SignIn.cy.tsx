    ...
    context('element existence', function () {
        ...
        context('sign-in form', function () {
            ...
            validations.forEach(v => {
                it(`has a field for "${v.label}"`, function () {
                    ...
                    //Test label
                    cy.get('[data-testid="sign-in"]').find(v.inputSelector)
            ...
            it('has a Forgot password link', function () {
                cy.get('[data-testid="sign-in"]')
                    .find('form.MuiBox-root').contains('button', 'Forgot your password?')
            })

            it('has a submit button', function () {
                cy.get('[data-testid="sign-in"]').contains('button[type=submit]', 'Sign in').should('exist')
            })
        })
        ...
        context('additional sign-in options', function () {
            ...
            it('has a sign in button with a Google account', function () {
                ...
                cy.contains('button', 'Sign in with Google').find('svg#google-icon').should('exist')
            })
            ...
            it('has a sign in button with a Facebook account', function () {
                ...
                cy.contains('button', 'Sign in with Facebook').find('svg#facebook-icon').should('exist')
            })
        })
    })
    ...
    context('functionality and events', function () {
        context('Forgot your password interactions', function () {
            it('launches a modal when clicked', function () {
                ...
                cy.get(`[id="forgot-password-dialog"]`).contains('button[type=submit]', 'Continue').should('exist')
                cy.get(`[id="forgot-password-dialog"]`).contains('button', 'Cancel').should('exist')
            })

            it('closes the modal on submission', function () {
                cy.contains('button', 'Forgot your password').click()
                cy.get(`[id="forgot-password-dialog"]`).find('input#email').type('onehandman@lannister.org')

                cy.get(`[id="forgot-password-dialog"]`).contains('button[type=submit]', 'Continue').click();
        ...
        context('Reset password form', function () {
            ...
            context('Email field', function () {
                it('does not allow empty input', function () {
                    ...
                    cy.get(`[id="forgot-password-dialog"]`).find('input#email')
            ...
            it('captures data when submitting the form', function () {
                ...
                */
                ...
                valueSets.forEach(values => {
                    ...
                    //Assert
                    cy.get('@log').invoke('getCalls').then(calls => {
                        //@ts-ignore
            ...
            context('Reset password form', function () {
                ...
                it('submits the email address on continuing', function () {
                    ...
                    */
                    ...
                    //Act
                    cy.get(`[id="forgot-password-dialog"]`).find('input#email').type(resetPasswordEmail)
                    cy.get(`[id="forgot-password-dialog"]`).contains('button', 'Continue').click()

                    //Assert
                    cy.get('@log').invoke('getCalls').then(calls => {
                        //@ts-ignore
                        expect(calls.some(c => Cypress._.isEqual(c.args[0], {resetPasswordEmail}))).to.eq(true)
                    })
                })
                ...
                it('does not submit on cancelling', function () {
                    ...
                    cy.get('@log').invoke('getCalls').then(calls => {
                        //@ts-ignore
                        expect(calls.some(c => Cypress._.isEqual(c.args[0], {resetPasswordEmail}))).to.eq(false)
                    })
                })
            })
        })
